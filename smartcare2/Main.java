...
public class Main {
    ...
    private static void doGenerateReport(HospitalServiceProxy service) {
        ...
        if (t.equals("1")) {
            strategy = new PatientReportStrategy();
        } else if (t.equals("2")) {
            strategy = new RoomReportStrategy();
        } else if (t.equals("3")) {
            strategy = new SectionReportStrategy();
        } else if (t.equals("4")) {
            strategy = new ProcedureReportStrategy();
        } else {
        ...
        String result = context.generateReport(service, data, fmt);
        if (result != null) {
            System.out.println("\n--- Report Preview ---\n" + result);
        }
    }
